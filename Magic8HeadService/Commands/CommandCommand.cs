...
public class CommandCommand : ICommandMbhToTwitch
{
    ...
    public IOptionsSnapshot<List<string>> commandNames { get; set; }
    ...
    {
        var registeredCommands = commandNames.Get("CommandNames");

        logger.LogInformation($"commands: {string.Join(',', registeredCommands)}");
